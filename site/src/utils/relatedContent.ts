...
function calculateSimilarity(
    ...
    if (post1.data.description && post2.data.description) {
        ...
        const commonDescWords = desc1Words.filter((word) => word.length > 4 && desc2Words.includes(word));
...
export function getRelatedPosts(
    ...
    const relatedWithScores = postsWithScores.filter(
        (item) => item.score > 0 && (item.commonTags === 0 || item.commonTags >= 2)
    );
